...
const STATUS_OPTIONS: { value: LeadStatus; label: string; cls: string }[] = [
  { value: "novo",        label: "Novo",        cls: "bg-blue-500/10 text-blue-600 border-blue-500/30" },
  { value: "em_contato",  label: "Em Contato",  cls: "bg-amber-500/10 text-amber-600 border-amber-500/30" },
  { value: "convertido",  label: "Convertido",  cls: "bg-emerald-500/10 text-emerald-600 border-emerald-500/30" },
  { value: "perdido",     label: "Perdido",     cls: "bg-muted text-muted-foreground border-border" },
]
          ...
          {filtered.length === 0 ? (
            <div className="py-16 flex flex-col items-center gap-3 text-center">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-surface elevation-soft">
                <MessageSquare size={20} strokeWidth={1.25} className="text-muted-foreground/50" />
              </div>
              <p className="font-serif text-base font-semibold text-foreground">Nenhum lead encontrado</p>
            </div>
          ) : (
            filtered.map((lead) => {
