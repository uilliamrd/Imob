...
export function InviteUserForm({ orgs }: { orgs: OrgOption[] }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/admin/invite", {
      ...
      body: JSON.stringify({ email, role, organization_id: orgId || null }),
    })

    const data = await res.json()

    if (res.ok) {
      setResult({ type: "success", message: `Convite enviado para ${email}. O usuário receberá um email para criar a senha.` })
      setEmail("")
      setRole("corretor")
      setOrgId("")
    } else {
      setResult({ type: "error", message: data.error ?? "Erro ao enviar convite." })
    }
