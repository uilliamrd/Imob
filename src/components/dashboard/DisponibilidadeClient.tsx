...
export function DisponibilidadeClient({ developments, properties: initial }: Props) {
  ...
  function renderTable(props: Property[]) {
    if (props.length === 0) {
      return <p className="text-muted-foreground/50 text-sm font-sans py-6 text-center">Nenhuma unidade cadastrada.</p>
    }
        ...
        <div className="flex gap-4 mb-4 px-1">
          {STATUS_OPTIONS.map((s) => (
            <div key={s.value} className="flex items-center gap-1.5">
              ...
              <span className="text-muted-foreground text-xs font-sans">{totals[s.value]} {s.label}</span>
            </div>
          ))}
        </div>

        {/* Header */}
        <div className="grid grid-cols-12 gap-3 px-4 py-2.5 text-[10px] uppercase tracking-[0.15em] text-muted-foreground/50 font-sans border-b border-border">
            ...
            return (
              <div key={p.id} className="grid grid-cols-12 gap-3 px-4 py-3 items-center hover:bg-white/[0.02] transition-colors">
                <div className="col-span-4">
                  <p className="text-foreground/80 text-sm font-sans">{p.title}</p>
                  {p.code && <p className="text-muted-foreground/50 text-[10px] font-sans">#{p.code}</p>}
                </div>
                <div className="col-span-2 text-center text-muted-foreground text-xs font-sans">
                ...
                <div className="col-span-2 flex justify-end">
                  <div className="relative group">
                    ...
                    {/* Dropdown on hover */}
                    <div className="absolute right-0 top-full mt-1 w-36 bg-[#1a1a1a] border border-border rounded-xl overflow-hidden shadow-2xl z-10 hidden group-hover:block">
                      {STATUS_OPTIONS.map((s) => (
                        <button key={s.value} onClick={() => updateStatus(p.id, s.value as Status)}
                          className={`w-full flex items-center gap-2 px-3 py-2.5 text-xs font-sans hover:bg-muted/50 transition-colors ${p.status === s.value ? "text-gold" : "text-foreground/60"}`}>
  ...
  return (
    <div className="space-y-4">
      {developments.map((dev) => {
        ...
        return (
          <div key={dev.id} className="bg-card border border-border rounded-2xl overflow-hidden">
            <button
            ...
            >
              <div className="flex items-center gap-3">
                {dev.cover_image
                  ? <Image src={dev.cover_image} alt="" width={36} height={36} className="w-9 h-9 rounded-lg object-cover border border-border" />
                  : <div className="w-9 h-9 rounded-lg bg-muted/50 flex items-center justify-center"><Building2 size={15} className="text-muted-foreground/50" /></div>
                }
                <div className="text-left">
                  <p className="text-foreground/80 text-sm font-sans font-medium">{dev.name}</p>
                  <p className="text-muted-foreground/60 text-xs font-sans">{devProps.length} unidades</p>
                </div>
              </div>
              {isExpanded ? <ChevronUp size={14} className="text-muted-foreground/50" /> : <ChevronDown size={14} className="text-muted-foreground/50" />}
            </button>
            {isExpanded && (
      ...
      {standalone.length > 0 && (
        <div className="bg-card border border-border rounded-2xl overflow-hidden">
          <div className="px-6 py-4 flex items-center gap-2 border-b border-border">
            ...
            <p className="text-muted-foreground text-sm font-sans">Sem empreendimento</p>
          </div>
      ...
      {developments.length === 0 && standalone.length === 0 && (
        <div className="py-16 text-center text-muted-foreground/50 font-sans">
