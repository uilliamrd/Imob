  ...
  if (items.length === 0) {
    return (
      <EmptyState
        ...
        description="Adicione imóveis na Base de Imóveis para montar seu portfólio."
        action={
          <Link href="/dashboard/vitrine"
            className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-xl bg-[var(--forest)] text-[var(--forest-foreground)] hover:opacity-90 transition-opacity">
            Ir para Base de Imóveis →
          </Link>
        }
      />
    )
  ...
  return (
    <div>
      ...
      {sorted.length === 0 && items.length > 0 && (
        <EmptyState compact icon={Home} title="Nenhum resultado" description="Tente outros filtros." />
      )}
      ...
      <div className="hidden sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {sorted.map((item) => (
          <div
            key={item.id}
            className={`group bg-card border rounded-2xl overflow-hidden transition-all duration-200 hover-lift ${
              item.is_featured
                ? "border-[color-mix(in_srgb,var(--gold)_40%,transparent)] elevation-card"
                : "border-border elevation-soft hover:border-[color-mix(in_srgb,var(--gold)_25%,transparent)]"
            }`}
          >
            <Link href={`/imovel/${item.property.slug}`} target="_blank" rel="noopener noreferrer"
            ...
            <div className="p-4">
              ...
              <p className="font-serif text-foreground text-sm font-bold mb-3">
                {formatPrice(item.property.price)}
              </p>
