    ...
    <div>
      ...
      <div className="flex gap-3 mb-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-card border border-border text-white placeholder-muted-foreground/40 pl-9 pr-4 py-3 rounded-xl font-sans text-sm focus:outline-none focus:border-gold/40 transition-colors"
          />
        </div>
        <button
          ...
          className={`flex items-center gap-2 px-4 py-3 rounded-xl border text-xs uppercase tracking-[0.15em] font-sans transition-colors ${
            showFilters || activeFilters > 0 ? "border-gold/50 text-gold bg-gold/10" : "border-border text-muted-foreground hover:border-white/20 hover:text-foreground/60"
          }`}
        >
      ...
      {showFilters && (
        <div className="bg-card border border-border rounded-xl p-4 mb-4 grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              {[null, 1, 2, 3, 4].map((n) => (
                <button key={n ?? "all"} onClick={() => setFilterDorms(n)}
                  className={`flex-1 py-1.5 rounded text-xs font-sans transition-colors ${
                    filterDorms === n ? "bg-gold text-graphite" : "bg-muted/50 text-muted-foreground hover:bg-muted"
                  }`}>
                  {n === null ? "Todos" : `${n}+`}
                </button>
              ))}
          ...
          <div>
            <label className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground font-sans block mb-1.5">Preço mín.</label>
          ...
          <div>
            <label className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground font-sans block mb-1.5">Preço máx.</label>
      ...
      {/* Count */}
      <p className="text-muted-foreground/50 text-xs font-sans mb-4">{filtered.length} imóveis encontrados · {listed.size} no seu portfólio</p>
      ...
      <div className="space-y-3 md:space-y-0 md:grid md:grid-cols-2 xl:grid-cols-3 md:gap-4">
        {filtered.map((p) => {
          ...
          return (
            <div key={p.id} className={`bg-card border rounded-2xl overflow-hidden transition-all duration-200 ${
              isListed ? "border-gold/30" : "border-border hover:border-border"
            }`}>
              ...
              <div className="md:hidden flex gap-3 p-3">
                {/* Square thumbnail */}
                <div className="relative w-24 h-24 flex-shrink-0 rounded-xl overflow-hidden bg-muted/50">
                  {p.images?.[0]
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between">
                  <div>
                    {p.code && <span className="text-muted-foreground/50 text-[9px] font-sans">#{p.code} · </span>}
                    <p className="font-serif text-foreground font-semibold text-[15px] leading-snug line-clamp-2">{p.title}</p>
                    {(p.neighborhood || p.city) && (
                      <p className="text-muted-foreground text-xs font-sans mt-0.5 flex items-center gap-1">
                        <MapPin size={9} />{p.neighborhood}{p.city ? `, ${p.city}` : ""}
                      </p>
                    )}
                    <div className="flex items-center gap-2.5 text-muted-foreground text-[11px] font-sans mt-1.5">
                    ...
                    <button
                      ...
                      className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-sans transition-all ${
                        isListed
                          ...
                          : "bg-muted/50 border border-border text-muted-foreground hover:bg-gold/10 hover:border-gold/30 hover:text-gold"
                      }`}
                    >
              ...
              <div className="hidden md:block">
                ...
                <div className="p-4">
                  {p.code && <div className="flex items-center gap-1 text-muted-foreground/50 text-[10px] font-sans mb-1"><Hash size={9} />{p.code}</div>}
                  <p className="font-serif text-foreground font-semibold text-base leading-tight mb-1">{p.title}</p>
                  {(p.neighborhood || p.city) && (
                  ...
                  {expanded === p.id && (
                    <div className="mt-3 pt-3 border-t border-border space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-1 text-[11px] font-sans text-muted-foreground">
      ...
      {filtered.length === 0 && (
        <div className="flex flex-col items-center gap-3 py-20 text-center">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-surface elevation-soft">
            <Search size={20} strokeWidth={1.25} className="text-muted-foreground/50" />
          </div>
          <p className="font-serif text-base font-semibold text-foreground">Nenhum imóvel encontrado</p>
          <p className="text-sm text-muted-foreground">Tente outros termos ou ajuste os filtros.</p>
        </div>
      )}
    </div>
  )
