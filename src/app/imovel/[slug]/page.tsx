...
import { DescriptionExpand } from "@/components/property/DescriptionExpand"
...
import type { Property, Organization, Development, PropertyFeatures } from "@/types/database"
...
export default async function ImovelPage({ params, searchParams }: PageProps) {
  ...
  // Similar properties (reuse existing client)
  ...
  if (property.city) {
    const { data } = await supabase
      ...
      .limit(3)
    similar = (data ?? []) as SimilarProperty[]
  }
      ...
      <div className="max-w-6xl mx-auto px-4 lg:px-6 py-6 lg:py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-16 mt-4 lg:mt-16">
          ...
          <div className="lg:col-span-2">
            ...
            <PropertyActionButtons slug={property.slug} title={property.title} />
            ...
            {property.description && (
              <div className="bg-card lg:bg-transparent rounded-2xl lg:rounded-none p-5 lg:p-0 mb-3 lg:mb-10 border border-border lg:border-0">
                ...
                <DescriptionExpand text={property.description} />
              </div>
            )}
              ...
              <div className="bg-card lg:bg-transparent rounded-2xl lg:rounded-none p-5 lg:p-0 mb-3 lg:mb-10 border border-border lg:border-0">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-2">
                  {property.tags.map((tag) => {
                    ...
                    return (
                      <div key={tag} className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl border border-border bg-card hover:border-[var(--gold)]/30 transition-colors">
                        <Icon size={13} className="text-[var(--gold)] flex-shrink-0" />
                        <span className="text-xs font-sans text-foreground/80 truncate">{info.label}</span>
                      </div>
                    )
                  })}
                </div>
